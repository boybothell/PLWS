#!/usr/bin/env python3
"""Launch 4-way vLLM shards for dense 4B Keep/Revise forecast."""
...
def main() -> None:
    ...
    out_dir = AE / "results/confcal_judge/v2/dense_forecast" / args.dataset
    ...
    for shard_id, gpu in enumerate(gpus):
        ...
        print(f"launch forecast {args.dataset} shard={shard_id} gpu={gpu}", flush=True)
        processes.append(subprocess.Popen(command, cwd=AE, env=env, stdout=log, stderr=subprocess.STDOUT))
        log.close()
    codes = [process.wait() for process in processes]
    if any(codes):
        raise RuntimeError(f"forecast failed {args.dataset}: {codes}")
    print(f"forecast complete {args.dataset}", flush=True)
