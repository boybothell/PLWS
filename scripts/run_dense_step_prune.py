#!/usr/bin/env python3
"""Launch 4-way HF shards for dense step-entropy prune-and-reanswer."""
...
SCRIPT = AE / "scripts/pilot_step_prune.py"
...
def main() -> None:
    ...
    out_dir = AE / "results/confcal_judge/v2/dense_step_prune" / args.dataset
    ...
    for shard_id, gpu in enumerate(gpus):
        ...
        print(f"launch step-prune {args.dataset} shard={shard_id} gpu={gpu}", flush=True)
        processes.append(subprocess.Popen(command, cwd=AE, env=env, stdout=log, stderr=subprocess.STDOUT))
        log.close()
    codes = [process.wait() for process in processes]
    if any(codes):
        raise RuntimeError(f"step-prune failed {args.dataset}: {codes}")
    print(f"step-prune complete {args.dataset}", flush=True)
