#!/usr/bin/env bash
# Full-dense Qwen3-4B open-think stop-margin. Default MATH, 4 GPUs, vLLM.
set -euo pipefail
AE="$(cd "$(dirname "$0")/.." && pwd)"
cd "$AE"
DS="${1:-math-500}"
...
OUTD="${AE}/results/confcal_judge/v2/dense_4b_open_think/${DS}"
GPUS="${GPUS:-2,3,4,5}"
...
echo "4b-open-think ${DS} n_cand=$(wc -l < "$CAND") gpus=${GPUS} backend=vllm"
pids=()
i=0
for gpu in "${GPU_ARR[@]}"; do
  CUDA_VISIBLE_DEVICES="$gpu" VLLM_LENS_DISABLE=1 LD_LIBRARY_PATH="$LD_LIBRARY_PATH" \
    "$PY" scripts/score_dense_4b_open_think.py \
      --candidates "$CAND" \
...
"${AE}/.venv/bin/python" scripts/analyze_dense_4b_open_think.py --dataset "$DS" --scores "$OUTD" \
  --out "${AE}/tables/probe_dense_4b_open_think_${DS}.md"
echo "4b-open-think ${DS} done"
