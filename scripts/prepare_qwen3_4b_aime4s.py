#!/usr/bin/env python3
"""Qwen3-4B AIME24/25 four-seed jobs for core / wait / core+."""
...
WINDOW_ROOT = ROOT / "results" / "runs" / "plws" / "window_first" / "k_4" / "lexicon_core" / "qwen3_4b"
JOBS_DIR = ROOT / "results" / "experiments" / "lexicon_ablation" / "jobs"
OUT_ROOT = ROOT / "results" / "experiments" / "lexicon_ablation" / "aime4s" / "qwen3_4b"
PRE_ROOT = ROOT / "results" / "experiments" / "lexicon_ablation" / "pre" / "qwen3_4b"
SEEDS = (42, 0, 1, 123)
DATASETS = ("aime24", "aime25")
KINDS = ("low", "mix", "high")
CONFIGS = ("core", "wait", "core_plus_let_me")
AIME_DATASETS = set(DATASETS)
...
def reuse_seed42(config: str) -> int:
    wanted = {
        row["uid"]
        for row in load_jsonl(JOBS_DIR / "aime4s_qwen3_4b_s42.jsonl")
    }
    copied: list[dict] = []
    seen: set[str] = set()
    folder = PRE_ROOT / config
    files = sorted(folder.glob("scores*.jsonl"))
    for path in files:
        for row in load_jsonl(path):
            uid = row.get("uid")
            ...
            copied.append(row)
            seen.add(str(uid))
    out = OUT_ROOT / "s42" / config / "scores.jsonl"
...
def main() -> None:
    ...
    for seed in SEEDS:
        ...
        path = JOBS_DIR / f"aime4s_qwen3_4b_s{seed}.jsonl"
