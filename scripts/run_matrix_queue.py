#!/usr/bin/env python3
"""Uninterrupted five-model then 30B PLWS-core + unified-host DEER queue."""
...
RUN_ROOT = ROOT / "results" / "runs" / "matrix_queue"
...
def command_for(task: MatrixTask) -> list[str]:
    if task.kind == "prereq":
        return ["bash", str(ROOT / "scripts" / "run_matrix_prereq_cell.sh")]
    if task.kind == "plws":
        return ["bash", str(ROOT / "scripts" / "run_matrix_plws_cell.sh")]
    if task.kind == "deer":
        return ["bash", str(ROOT / "scripts" / "run_deer_official.sh")]
    raise ValueError(task.kind)


def env_for(task: MatrixTask, gpus: tuple[str, ...]) -> dict[str, str]:
    env = os.environ.copy()
    env.update(
        {
            "PLWS_ROOT": str(ROOT),
...
def adopt_running(tasks: list[MatrixTask]) -> dict[int, Running]:
    ...
    for proc in Path("/proc").iterdir():
        ...
        task: MatrixTask | None = None
        if any(part.endswith("run_matrix_prereq_cell.sh") for part in cmd):
            task_id = (
                f"prereq__{env.get('MODEL_TAG', '')}__"
                f"{env.get('DATASET', '')}__s{env.get('SEED', '')}"
            )
            task = wanted.get(task_id)
        elif any(part.endswith("run_matrix_plws_cell.sh") for part in cmd) or (
            ...
            try:
                ...
                kind = env.get("KIND") or cmd[cmd.index("--run-kind") + 1]
...
def launch(task: MatrixTask, assigned: tuple[str, ...], attempt: int) -> Running:
    LOG_ROOT.mkdir(parents=True, exist_ok=True)
...
def wait_for_blockers(poll: int) -> None:
    while True:
        idle, reason = blockers_idle(PATHS)
        if idle:
            event("blockers_idle", reason=reason)
            return
        write_status([], {}, [], [], None, waiting=reason)
        event("waiting_blockers", reason=reason)
...
def validate(gpus: tuple[str, ...]) -> None:
    ...
    if {"5", "7"} & set(gpus):
        raise ValueError("GPU 5/7 stay unused")
    for model, path in MODELS.items():
        if not (Path(path) / "config.json").is_file():
            raise FileNotFoundError(f"model missing: {model} {path}")
    for script in (
        ROOT / "scripts" / "run_matrix_prereq_cell.sh",
        ROOT / "scripts" / "run_matrix_plws_cell.sh",
        ROOT / "scripts" / "run_deer_official.sh",
    ):
        if not script.is_file():
            raise FileNotFoundError(script)
    for dataset in ("math-500", "olympiadbench", "gpqa-diamond", "aime24", "aime25"):
...
def main() -> int:
    ...
    parser.add_argument("--no-thirty", action="store_true")
    parser.add_argument("--no-wait-current", action="store_true")
    parser.add_argument("--until-phase", choices=PHASES)
    parser.add_argument(
        "--models",
        default="",
        help="Comma-separated model tags. Overrides --no-thirty when set.",
    )
    ...
    if args.models.strip():
        ...
        chosen_models = ALL_MODELS if not args.no_thirty else FIVE_MODELS
        tasks = build_tasks(PATHS, include_thirty=not args.no_thirty)
        ...
        for model in ("qwen3_4b", "qwen3_8b"):
            ...
            for seed in (42, 0, 1, 123):
                ...
                if any(copied.values()):
                    event("imported_aime4s", model=model, seed=seed, copied=copied)
            ...
            for dataset in DATASETS:
                for seed in SEEDS:
                    ...
                    if needs_cpu_export(PATHS, model, dataset, seed):
                        ...
                        event("exported_jobs", model=model, dataset=dataset, seed=seed)

    if args.until_phase:
        stop_at = PHASES.index(args.until_phase)
        tasks = [task for task in tasks if PHASES.index(task.phase) <= stop_at]
    counts = summarize(tasks)
    if args.dry_run:
        print(
            json.dumps(
                {"gpus": list(gpus), "models": list(chosen_models), "counts": counts},
                ensure_ascii=False,
                indent=2,
            )
        )
    ...
    if not args.no_wait_current:
        event("waiting_blockers", paths=list(BLOCKER_STATUS))
        wait_for_blockers(args.poll)
