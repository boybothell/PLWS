#!/usr/bin/env python3
"""Tian-style per-candidate P(A) along 7B k=4 same-answer windows."""
...
ROOT = AE / "results/hc_verbal_ind/olympiadbench_s42"
TABLE = AE / "tables/hc_verbal_ind_oly.md"
OK = {"ok", "high_ok"}
...
def main() -> None:
    ...
    lines = [
        "# 4B 逐项口头分：7B 奥赛四步同答对窗 / 错窗",
        "",
        "Tian top-k / Wang dict：一次给出每个候选各自的 P(correct)∈[0,1]，不要求和为 1。",
        "提示只有题目、该步 7B 思路、A/B/C 原文和 D. None of the above。",
        f"前向 {len(rows)}，有效 {len(ok)}，解析失败 {len(fail)}，超长 {len(long)}。"
        f"齐四步的窗 {len(wins)}。",
        "",
        "假说：对窗四步 P(A) 都高；错窗扣不住。",
        "",
        ...
        "",
        "格式几乎都合法，但中位 P(A) 贴 0，质量常堆在 D。对/错 AUROC≈0.5，假说这边没活。",
        "",
    ]
    TABLE.write_text("\n".join(lines))
    print("\n".join(lines))
